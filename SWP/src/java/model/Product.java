...
public class Product {
    ...
    public Product(String id, String name, float price, String image, int quantity, String wire, String description, String releasedate) {
        this.id = id;
        this.name = name;
        this.price = price;
        this.image = image;
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (rs.getDate(9) != null) {
                    date = f.format(rs.getDate(9));
                }

                list.add(new Product(Tid, Tname, Tprice, Timage, Tquantity, Twire, Tdescription, date));
            }
        } catch (Exception ex) {
            System.out.println("getAllProduct" + ex.getMessage());
        }
    ...
    public void setDiscount(String start, String end, int discount, String id) {
        try {
            String add = "insert into headphone.discount (DiscountID, StartSaleDate, EndSaleDate, Discount, ProductID) values (?, ?, ?, ?, ?)";
            pstm = cnn.prepareStatement(add);
            int disid = maxDiscountID();
            pstm.setInt(1, disid);
    ...
    public int maxDiscountID() {
        try {
            String sql = "SELECT COALESCE(MAX(DiscountID), 0) FROM headphone.discount ";
            pstm = cnn.prepareStatement(sql);
            rs = pstm.executeQuery();
            ...
            System.out.println("maxDiscountID: " + e.getMessage());
        }
        return 0;
    }
